...
function createCartDropdownItem(item) {
  ...
  itemDiv.innerHTML = `
    <img src="${item.imageUrl}" alt="${item.name}" class="cart-dropdown-item-image" onerror="this.src='https://via.placeholder.com/60?text=No+Image'">
...
function createCartPageItem(item) {
  ...
  itemDiv.innerHTML = `
    <div class="cart-page-item-image">
      <img src="${item.imageUrl}" alt="${item.name}" onerror="this.src='https://via.placeholder.com/120?text=No+Image'">
    </div>
...
function renderCartSummary(cart) {
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener('click', () => {
      // Redirect to payment/checkout page (you can customize this)
