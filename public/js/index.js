// Fetch and display popular products from Firestore
...
/**
 * Fetches popular products from Firestore and displays them
 */
async function loadPopularProducts() {
  try {
    ...
    // Fetch products where popular is true and status is active
    // Note: This assumes 'popular' is a boolean field in Firestore
    // If 'popular' is stored differently (e.g., as a string "true"), adjust the query accordingly
    ...
    try {
      ...
      productsSnapshot = await db
        ...
        .limit(4) // Limit to 4 products to match the current layout
        .get();
    } catch (error) {
...
async function loadNewProducts() {
  try {
    ...
    // Fetch 3 most recent perfume products
    let perfumeProducts = [];
    try {
      const perfumeSnapshot = await db
        .collection('products')
      ...
      perfumeSnapshot.forEach((doc) => {
        perfumeProducts.push({ id: doc.id, ...doc.data() });
      });
    } catch (error) {
      if (error.code === 'failed-precondition') {
    ...
    // Fetch 3 most recent clothes products
    let clothesProducts = [];
    try {
      ...
      clothesSnapshot.forEach((doc) => {
        clothesProducts.push({ id: doc.id, ...doc.data() });
      });
    } catch (error) {
      if (error.code === 'failed-precondition') {
        // Try without orderBy if index doesn't exist
  ...
  try {
    ...
    try {
      const popularSnapshot = await db
        ...
        .where('status', '==', 'active')
        .orderBy('createdAt', 'desc')
        .limit(10) // Get multiple popular products for carousel
        .get();
      
      popularSnapshot.forEach((doc) => {
        popularProducts.push({ id: doc.id, ...doc.data() });
      });
    } catch (error) {
      if (error.code === 'failed-precondition') {
        // Try without orderBy if index doesn't exist
    ...
    try {
      ...
      if (!perfumeSnapshot.empty) {
        const doc = perfumeSnapshot.docs[0];
        lastPerfumeProduct = { id: doc.id, ...doc.data() };
      }
    } catch (error) {
      if (error.code === 'failed-precondition') {
        // Try without orderBy if index doesn't exist
    ...
    try {
      const clothingSnapshot = await db
        .collection('products')
        .where('category', '==', 'clothes')
        .where('status', '==', 'active')
        .orderBy('createdAt', 'desc')
      ...
      if (!clothingSnapshot.empty) {
        const doc = clothingSnapshot.docs[0];
        lastClothingProduct = { id: doc.id, ...doc.data() };
      }
    } catch (error) {
      if (error.code === 'failed-precondition') {
        // Try without orderBy if index doesn't exist
